 ...
 * first name, and lname for the last name. Each field will be a string of
 * size 20 (characters).
...
func main() {
	...
	names := make([]Name, 0, 10) //Randomly chosen capacity

	// Using ReadFile because its mandatory
	// There are better ways to iterate per line
	barr, _ := ioutil.ReadFile(filepath)
	content := strings.Fields(string(barr))

	for i := 0; i < len(content); i = i + 2 {
		name := Name{fname: content[i], lname: content[i+1]}
